...
Each per-build section below has the same three parts. The **wire delta** is what
claustrum must match byte-for-byte. **Off-wire churn** is any source that moved
but never reaches the JSON-RPC surface. **How it was bounded** gives the
measurement that confirmed that nothing else changed.
...
**Wire delta.** Method count **18 → 19**. There are five changes. All five are
matched byte-for-byte (values and timing) against the reference:
...
and it calls 15 of the 19 methods. It does not call `process.killAndWait`. It
also does not call `server.version` or `server.shutdown`, which the `--stop` CLI
drives. A capture of a real session will therefore not exercise the new method, and
the synthetic battery stays the gate for it. That client also bears on D1's trust
...
- The `-install` path gained an HTTPS download, a SHA-256 verify and a robust
  ...
  wire impact.
- A refactor of the `process.Spawn` failure path. This change is on a
  wire-reachable path, the spawn failure. It was therefore the killAndWait-shaped
  risk: a change that the happy-path battery never stresses.
