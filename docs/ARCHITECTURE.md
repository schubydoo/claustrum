...
- **What rests on it:** **D3, D5, D10, D11 and D12** — every "why it stopped being
  always-on" argument turns on the person who pays having no way to decline. The
  claim is load-bearing on both argv surfaces: D3 and D5 are `-serve` flags, D10,
  D11 and D12 are `-install` ones — plus
  ...
  **D-numbered** entry. (The CT block uses the tag in a looser "off unless asked
  ...
  The trigger is recorded once here rather than six times, because it is one claim
