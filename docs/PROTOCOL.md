...
- The cap **shipped on by default at 512 MiB** and that was a live user-facing
  ...
  *Driver claims and their provenance*. ⚠️ The evidence is a look at the setup UI
  ...
  on one cold start (2026-08-08, with the `-install` a cache hit) — but Desktop's
  own config files and any forwarded environment were never enumerated, and a
  config file it turns into argv is one of the two routes that would falsify it). Flipping the default to `0` is the parity
